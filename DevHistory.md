...
- **比 SM89 强**：有部分 FP4/FP8 硬件指令
- **比 SM100 弱**：缺 TMA、缺 tcgen05、`.tile::scatter4` 不存在
- **结果**：数据中心卡的生态（vLLM/DeepGEMM）不认它，消费卡的生态（llama.cpp）还在追赶
- DeepGEMM 明确不打算支持 SM120 系列
- Triton 标准 FP4 MoE kernel 在 SM120 上编译失败（生成 SM100 专属 PTX 指令）
- Marlin 是唯一能跑的 FP4 MoE backend，但计算结果部分错误

**Consumer-DeepGEMM 是目前全世界唯一在 SM120 系列上正确运行 V4 Flash FP4 MoE 的方案。**
...
**待验证**（回家后执行）：
1. clone christopherowen 的三个 repo
2. 构建 Docker 镜像
3. 用 V4 Flash 权重跑长输出中英文测试
4. 对比 Consumer-DeepGEMM（1.87 tok/s）和 jasl fork（14 tok/s）的速度和正确性

如果能跑通，速度可能从 1.87 tok/s 直接飙到两位数——这就是 SM121 上 V4 Flash 的终极方案。
