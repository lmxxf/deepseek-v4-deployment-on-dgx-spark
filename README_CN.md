...
| 场景 | 状态 | 速度 |
|------|------|------|
| TP=2，中文问答/写作/代码 | **完美** | **~12 tok/s** |
| TP=2，英文问答/代码 | **完美** | **~12 tok/s** |
| PP=2，中文/英文/代码 | **可用** | **~10-11 tok/s** |
...
基于 jasl/vllm `ds4-sm120` + 混合后端补丁（2026-05-11），跳到第 4 步启动服务，使用 `-t lmxxf/vllm-deepseek-v4-dgx-spark`。

如果要尝试 PP=2（流水线并行），需要基于已上传的镜像生成一个很小的补丁镜像。这个镜像只覆盖 `mxfp4.py`，让 layer 42 的 DeepGEMM 回退在 PP 路径下也能匹配到正确的 layer name：
...
PP=2 已验证可用，但单请求生成速度不如 TP=2：PP=2 大约 10-11 tok/s，TP=2 大约 12 tok/s。它适合作为流水线并行基线，或者后续测试高并发/大 batch 时使用，不是当前默认最快方案。
...
PP 特有要求：
- `vllm-deepseek-v4-pp-fix`：派生补丁镜像，修复 `mxfp4.py` 里的 layer name 解析；否则 layer 42 回退可能打不中，输出会乱码
- `VLLM_USE_RAY_V2_EXECUTOR_BACKEND=1`：必须打开；默认 Ray compiled graph PP 路径可能在第二个请求卡死
- `--tensor-parallel-size 1 --pipeline-parallel-size 2`：按层切分到两台机器，而不是每层内部按张量切分

PP=2 预期稳态速度：

| 测试 | 输出 | 速度 |
|------|------|------|
| `17 * 23 = ?` | `391` | warmup 后亚秒级 |
| 中文写作 | 正确 | ~10-11 tok/s |
| 英文代码 / quicksort | 正确 | ~10-11 tok/s |
...
PP=2 的显存规模和 TP=2 接近：TP=2 是每台机器持有全部 43 层、但每层张量切半；PP=2 是每台机器只持有一半层、但每层张量完整。实测 PP rank0 模型权重约 74.8 GiB，PP rank1 约 71.9 GiB。
...
| 混合后端 + PP=2 | 正确 | 正确 | 10-11 tok/s | 可用，作为 PP 基线 |

## PP=2 踩坑

PP=2 不是简单把启动参数从 `--tensor-parallel-size 2` 改成 `--pipeline-parallel-size 2` 就结束。

默认 Ray PP compiled graph 路径在这个组合下会出现“第一个请求能返回，第二个请求卡死”的问题。解决方法是打开：

```bash
VLLM_USE_RAY_V2_EXECUTOR_BACKEND=1
```

另外，185 期的 layer 42 回退逻辑最初只匹配 TP 路径下的 layer name。PP 路径下实际名字可能是 `model.layers.42.ffn.experts` 或 `model.layers.42.ffn`，旧正则打不中，导致最后一层继续走 Marlin，输出乱码。`vllm-deepseek-v4-pp-fix` 镜像修复了这个解析问题。

最终结论：DeepSeek V4 的 PP 主体不用重写；需要修的是 mixed backend 补丁对 PP layer name 的兼容，以及 Ray executor 路径选择。
