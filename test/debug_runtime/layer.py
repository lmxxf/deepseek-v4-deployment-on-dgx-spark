        ...
        for expert_name, loaded_weight in weights:
            ...
            for param_name, weight_name, expert_id, shard_id in expert_mapping:
                ...
                for expert_id, loaded_expert in enumerate(loaded_experts, start=start):
                    logger.warning(
                        "MOE_LOAD_BEGIN rank=%s layer=%s expert=%d shard=%s param=%s weight=%s loaded_shape=%s loaded_dtype=%s param_shape=%s param_dtype=%s",
                        self.tp_rank,
                        self.layer_name,
                        expert_id,
                        shard_id,
                    ...
                    if success:
                        logger.warning(
                            "MOE_LOAD_DONE rank=%s layer=%s expert=%d shard=%s param=%s",
                            self.tp_rank,
                            self.layer_name,
                            expert_id,
                            shard_id,
