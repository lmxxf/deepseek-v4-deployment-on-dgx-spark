...
def _debug_hidden_layers_enabled(layer_idx: int) -> bool:
    spec = os.environ.get("VLLM_DSV4_DEBUG_HIDDEN_LAYERS")
    if not spec:
        return False
    for item in spec.split(","):
        ...
        if ":" in item:
            ...
            end = int(end_s) if end_s else 10**9
            if start <= layer_idx < end:
                return True
        elif int(item) == layer_idx:
            return True
    return False


def _debug_log_tensor_stats(tag: str, tensor: torch.Tensor) -> None:
    if not os.environ.get("VLLM_DSV4_DEBUG_HIDDEN_STATS"):
        return
    max_tokens = int(os.environ.get("VLLM_DSV4_DEBUG_HIDDEN_MAX_TOKENS", "0"))
    if max_tokens > 0 and tensor.shape[0] > max_tokens:
        return
    with torch.no_grad():
        x = tensor.detach().float()
        logger.warning(
            "DSV4 hidden stats %s shape=%s mean=%.6e std=%.6e "
            "max_abs=%.6e rms=%.6e",
            tag,
            tuple(tensor.shape),
            x.mean().item(),
            ...
            x.abs().max().item(),
            torch.sqrt(torch.mean(x * x)).item(),
        )
...
class DeepseekV4DecoderLayer(nn.Module):
    def __init__(
        ...
        self._debug_hidden_stats_seen = 0
    ...
    def forward(
        ...
        if _debug_hidden_layers_enabled(self.layer_idx):
            max_seen = int(os.environ.get("VLLM_DSV4_DEBUG_HIDDEN_MAX", "2"))
            if self._debug_hidden_stats_seen < max_seen:
                _debug_log_tensor_stats(f"layer{self.layer_idx}.after_attn", x)
        ...
        if _debug_hidden_layers_enabled(self.layer_idx):
            max_seen = int(os.environ.get("VLLM_DSV4_DEBUG_HIDDEN_MAX", "2"))
            if self._debug_hidden_stats_seen < max_seen:
                _debug_log_tensor_stats(f"layer{self.layer_idx}.after_ffn", x)
                self._debug_hidden_stats_seen += 1
        return x
...
class DeepseekV4Model(nn.Module):
    ...
    def forward(
        ...
        _debug_log_tensor_stats("hc_head.out", hidden_states)
        hidden_states = self.norm(hidden_states)
        _debug_log_tensor_stats("model.norm.out", hidden_states)
